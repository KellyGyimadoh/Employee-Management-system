...
class Payroll extends Dbconnection
{
    ...
    protected function InsertNewPayrollRecord(){
        try {
            ...
            if ($result['count'] == 0) {
                ...
                $stmt->bindValue(':dueDate', "$nextMonth-28"); // Example: End of the month
                if($stmt->execute()){

                    return['success'=>true, 'message'=>"Payroll records for $nextMonth created."];
                }
             else {
                return['success'=>false, 'message'=>"Failed to insert payroll record please try again"];
             }
                
            }
            else{
                return['success'=>false, 'message'=>"Payroll records for $nextMonth already exist"];
