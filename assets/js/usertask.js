...
document.addEventListener("DOMContentLoaded", () => {
  ...
  taskTable.addEventListener("submit", async (e) => {
          ...
          if (e.target && e.target.matches("form[id^='unmarktask-form']")) {
              e.preventDefault();
  
              const salaryform = e.target;
              const resultData = await processForm(salaryform, '../../api/tasks/process.unmarktask.php');
              handleFormMessage(resultData);
          }
    ...
    (async () => {
        const tasksData = await fetchData('../../api/tasks/process.fetchusertask.php',
             currentPage, recordsPerPageforTask, searchQuery, userId, searchdate,taskstatus)
        if (tasksData?.tasks) {

            rendertaskTable(tasksData.tasks)
            renderTasksPaginator(tasksData.pagination.total_pages, tasksData.pagination.current_page)
            totalTask.innerHTML=tasksData.total_tasks
    ...
    function paymentForm(id, status,userid=null,duedate=null) {
        ...
        switch (status) {
            ...
            case 2:
              ...
              <form id='unmarktask-form-${id}' method='post'>
                <input type='hidden' name='id' value='${id}'/>
                 <input type="hidden" name="csrf_token" value='${getCsrfToken()}'>
                <button  class='btn btn-primary'>Unmark</button>
                </form>
            ...
            case 3:
                ...
                <button role='button'  class='btn btn-danger '>Penalty</button>
