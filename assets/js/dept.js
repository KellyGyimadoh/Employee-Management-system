...
document.addEventListener("DOMContentLoaded", () => {
   ...
   (async ()=>{
    const deptData= await fetchData('../../api/departments/process.viewalldpt.php',currentPage,recordsPerPageforDept,searchQuery)
    if(deptData){
...
recordsPerPageDpt.addEventListener("change",(e)=>{
    ...
    currentPage=1
    (async ()=>{
        const deptData= await fetchData('../../api/departments/process.viewalldpt.php',currentPage,recordsPerPageforDept,null)
        if(deptData){
