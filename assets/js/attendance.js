...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const searchAttendanceForm = document.getElementById("searchAttendance");
    const paginator = document.querySelector(".attendancepagination")


    attendanceTable.addEventListener("submit", async (e) => {
        if (e.target && e.target.matches("form[id^='markattendance-form']")) {
            e.preventDefault();
            const form = e.target;
    ...
    (async () => {
        const attendanceData = await fetchData('../../api/attendance/process.fetchallattendance.php', currentPage, recordsPerPageforAttendance, searchQuery, null, searchdate)
    ...
    function paymentForm(id, status) {
        ...
        switch (status) {
            case 1:
                attendanceform = `
                <form id='markattendance-form-${id}' method='post'>
