...
document.addEventListener("DOMContentLoaded", () => {
  ...
  taskTable.addEventListener("submit", async (e) => {
    ...
    (async () => {
        const tasksData = await fetchData('../../api/tasks/process.fetchalltasks.php', currentPage, recordsPerPageforTask, searchQuery, null, searchdate,taskstatus)
        if (tasksData?.tasks) {
    ...
    recordsPerPageTask.addEventListener("change", (e) => {
        ...
        currentPage = 1
            (async () => {
                const tasksData = await fetchData('../../api/tasks/process.fetchalltasks.php',
    ...
    searchTaskForm.addEventListener("submit", (e) => {
        ...
        let recordsPerPageforTask = recordsPerPageTask.value;
        (async () => {
            const tasksData = await fetchData('../../api/tasks/process.fetchalltasks.php', currentPage,
                 recordsPerPageforTask, searchQuery, null, searchdate,taskstatus)
            if (tasksData?.tasks) {
